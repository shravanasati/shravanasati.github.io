...
Qdrant had a really good documentation, and the guides cover numerous real world tasks. All these techniques mentioned above are written in Qdrant docs. [link1](https://qdrant.tech/documentation/fastembed/fastembed-optimize/) [link2](https://qdrant.tech/course/essentials/day-4/large-scale-ingestion/)
...
Using the averaged vector, a similarity search is performed on the vector DB. The search query has filters for NSFW genres, and against the anime IDs user has already watched.
...
After calculating the base scores, the ranker also diversifies the selection by not allowing genre overlap of more than 4 titles.


### Conclusion

I am happy with the current implementation. I might train it on user-anime interactions later to improve the ranker. I also need to experiment with the weights for retriever to find the optimal balance.
